...
**Incremental engine** — Walks the edit region of the previous tree and reuses unchanged content by reference. Reuse covers leaf nodes, the unchanged suffix, the untouched root, and — on a clean old tree — unchanged top-level siblings after the edited node. Admission for top-level sibling reuse is per node: a fragility bit plus byte-range equality, not a language allowlist. This block-splice keeps clean top-level edits, Go and CSS alike, at O(edit) cost, not O(file). Interior, non-top-level subtree reuse does not ship yet; it stays on the roadmap. External scanner state is serialized on each node boundary, so scanner-dependent content can be reused without replaying the scanner from the start.
