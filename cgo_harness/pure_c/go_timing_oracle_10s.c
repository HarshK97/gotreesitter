...
/* Raised from 750ms (run4) to 5s (run5) to 10s (run6) to shrink
 * single-fixture sampling noise in the A/A self-null check on shared-tenancy
 * SEV Confidential Space hosts; see gts-cs-bench-harness README run4 -> run6
 * notes. run5 at 5s still missed the +/-2% gate (worst case: C-oracle side
 * grammargen_lr at 5.19%, production side rewrite at 3.27%). */
static const uint64_t k_min_elapsed_ns = UINT64_C(10000000000);
